 ...
 */
export function extract(salt: Bytes, ikm: Bytes): Bytes {
  // RFC 5869 2.2: an absent salt is HashLen zero bytes.
  return hmac(salt.length > 0 ? salt : new Uint8Array(HASH_BYTES), ikm);
}

export function expand(prk: Bytes, info: Bytes, length: number): Bytes {
 ...
 */
export function labeledExtract(suiteId: Bytes, salt: Bytes, label: string, ikm: Bytes): Bytes {
  return extract(salt, concat(HPKE_VERSION, suiteId, utf8(label), ikm));
}

/** RFC 9180 section 4: LabeledExpand. */
export function labeledExpand(suiteId: Bytes, prk: Bytes, label: string, info: Bytes, length: number): Bytes {
