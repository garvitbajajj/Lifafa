...
import { labeledExpand, labeledExtract } from './hkdf.js';
import { rawPrivate, rawPublic, x25519PrivateKey, x25519PublicKey } from './keys.js';
 ...
 *
 * Every line here is checked against RFC 9180 appendix A.1 in test/hpke.test.js.
 */
...
export const X25519_SHA256_AES256GCM = {
...
export const X25519_SHA256_AES128GCM = {
  kemId: KEM_X25519_HKDF_SHA256,
  kdfId: KDF_HKDF_SHA256,
...
const suiteId = (suite) => concat(utf8('HPKE'), u16(suite.kemId), u16(suite.kdfId), u16(suite.aeadId));
const kemSuiteId = (suite) => concat(utf8('KEM'), u16(suite.kemId));

function keyBytes(suite) {
  switch (suite.aeadId) {
...
/** @returns {{privateKey: Uint8Array, publicKey: Uint8Array}} both raw, 32 bytes each */
...
export function deriveKeyPair(suite, ikm) {
...
function dh(privateScalar, publicKey) {
...
function extractAndExpand(suite, dhResult, kemContext) {
  const id = kemSuiteId(suite);
 ...
 * @returns {{enc: Uint8Array, sharedSecret: Uint8Array}} enc travels on the wire; the secret does not
 */
export function encapsulate(suite, recipientPublicKey) {
 ...
 */
export function encapsulateWith(suite, recipientPublicKey, ephemeralPrivateKey) {
...
/** Decap(enc, skR). */
export function decapsulate(suite, recipientPrivateKey, enc) {
...
export function keyScheduleContext(suite, info) {
 ...
 * @returns {{key: Uint8Array, baseNonce: Uint8Array, exporterSecret: Uint8Array}}
 */
export function keySchedule(suite, sharedSecret, info) {
  const id = suiteId(suite);
...
export function nonce(baseNonce, sequence) {
  ...
  for (let i = 0; i < 8; i++) {
    out[out.length - 8 + i] ^= seq[i];
  }
  return out;
}

/** Section 5.3: Export(exporter_context, L). */
export function exportSecret(suite, exporterSecret, exporterContext, length) {
...
export function seal(suite, recipientPublicKey, info, aad, plaintext) {
...
export function open(suite, recipientPrivateKey, enc, info, aad, ciphertext) {
