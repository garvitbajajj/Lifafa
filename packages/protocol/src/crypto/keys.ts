...
import { type Bytes, concat, unhex } from '../bytes.ts';
...
function require32(raw: Bytes, what: string): void {
  if (raw.length !== KEY_BYTES) throw new Error(`${what} must be ${KEY_BYTES} bytes, got ${raw.length}`);
}

export function x25519PublicKey(raw: Bytes): KeyObject {
...
export function x25519PrivateKey(scalar: Bytes): KeyObject {
...
export function ed25519PublicKey(raw: Bytes): KeyObject {
...
export function ed25519PrivateKey(seed: Bytes): KeyObject {
...
export function rawPublic(key: KeyObject): Bytes {
...
export function rawPrivate(key: KeyObject): Bytes {
